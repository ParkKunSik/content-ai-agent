...
*   **AI Platform:** Google Vertex AI, Gemini API, OpenAI (멀티 Provider 지원)
...
```bash
# Provider 선택 (VERTEX_AI | GEMINI_API | OPENAI)
LLM_PROVIDER=VERTEX_AI

# Google Vertex AI 설정 (LLM_PROVIDER=VERTEX_AI인 경우)
GCP__PROJECT_ID=your-project-id
GCP__REGION=asia-northeast3
GCP__CREDENTIALS_PATH=path/to/credentials.json
VERTEX_AI__MODEL_ADVANCED=gemini-2.5-pro
VERTEX_AI__MODEL_STANDARD=gemini-2.5-flash

# Gemini API 설정 (LLM_PROVIDER=GEMINI_API인 경우)
GEMINI_API__API_KEY=AIzaSy...your-api-key
GEMINI_API__MODEL_ADVANCED=gemini-2.5-pro
GEMINI_API__MODEL_STANDARD=gemini-2.5-flash

# OpenAI 설정 (LLM_PROVIDER=OPENAI인 경우)
OPENAI__API_KEY=sk-xxxxx
OPENAI__ORG_ID=org-xxxxx        # Optional
OPENAI__MODEL_ADVANCED=gpt-4o
OPENAI__MODEL_STANDARD=gpt-4o-mini
```

> **Note:** Provider 전환은 `LLM_PROVIDER` 환경변수만 변경하면 됩니다. 코드 수정 불필요.
>
> **환경변수 네이밍:** pydantic-settings의 `env_nested_delimiter="__"` 설정으로 중첩 구조 지원.
