...
class AsyncGenAISession:
    ...
    def generate_content(self, prompt: str) -> 'types.GenerateContentResponse':
        ...
        # GenerateContentResponse 고유 정보 로깅 (저수준)
    ...
    async def send_message(self, message: str) -> 'types.GenerateContentResponse':
        """
    ...
    def _log_response_metadata(self, response: 'types.GenerateContentResponse'):
                ...
                for i, candidate in enumerate(response.candidates):
                    if hasattr(candidate, 'finish_reason') and candidate.finish_reason:
                        ...
                        if finish_reason in problematic_reasons:
                            logger.warning(f"LLM finish_reason alert: {finish_reason} (model: {self._model_name}, candidate: {i})")
                        else:
                            logger.debug(f"LLM finish_reason: {finish_reason} (model: {self._model_name}, candidate: {i})")
