...
기존 GenerationViewer.generate_amazon_style_html()을 참고하되,
정제된 결과(RefineResult) 구조에 맞게 수정되었습니다.

주요 차이점:
- 입력: StructuredAnalysisRefineResult (정제된 데이터)
- CategoryItem: positive_count/negative_count (int) 사용 (배열 대신)
"""
import html
import re
...
class RefineResultRenderer:
    ...
    def generate_amazon_style_html(
        cls,
        result: StructuredAnalysisRefineResult,
        project_id: int,
        content_type: str,
        executed_at: str,
        ...
        """
        StructuredAnalysisRefineResult 데이터를 Amazon 스타일 HTML로 변환
...
"""
        ...
        for idx, category in enumerate(categories):
            sentiment = (
                category.sentiment_type.value
                if hasattr(category.sentiment_type, "value")
                else str(category.sentiment_type)
            )
